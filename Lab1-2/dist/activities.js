...
if (!activitiesSection)
    throw new Error("Cannot find activities section");
const carouselSection = document.createElement("section");
carouselSection.id = "carousel-section";
carouselSection.setAttribute("aria-label", "Карусель ілюстративних фото напрямів");
activitiesSection.parentElement.insertBefore(carouselSection, activitiesSection);
...
if (!searchInput)
    throw new Error("Cant find search input");
searchInput.addEventListener("input", (event) => {
    ...
    const arr = Object.keys(activitiesObj).filter((el) => el.toLowerCase().includes(search));
    console.log(arr);
    if (arr[0]) {
        const htmlId = activitiesObj[arr[0]];
        const idSplitter = htmlId.split("#");
        if (idSplitter.length != 2 || !idSplitter[1])
            throw new Error("Wrongly typed id");
